<template>
  <div class="role">
    <el-row justify="space-between" class="flex flex-only-center pad-20">
      <div class="title">
        产品权限 / <span>{{ state.roleNameQuery }}</span>
      </div>
    </el-row>
    <div class="roleList">
...
const getDetail = async () => {
  ...
  if (res.status.success) {
    state.permissionList = res.body.dropdownMenuList
...
// 全选
const handleCheckAllChange = async (val: any, item: any) => {
  // 设置全选数值
  const resourceIds = []
...
// 选中
const handleCheckedCitiesChange = (value: string | any[], index: number) => {
